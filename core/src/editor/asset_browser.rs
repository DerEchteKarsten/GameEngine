use std::{fs::FileType, mem::{replace, swap, take}, path::{Path, PathBuf}};

use anyhow::{Result, anyhow};
use bevy::{asset::{AssetServer, Handle, UntypedHandle}, ecs::{resource::Resource, system::{Local, ResMut}}, log, window::FileDragAndDrop};
use glam::{UVec2, Vec2};
use imgui::{DragDropFlags, StyleVar};

use crate::{assets::mesh::{GpuMesh, Scene}, ui::UiBuilder};

#[derive(Resource)]
pub struct AssetDND(pub Option<UntypedHandle>); 

//TODO make faster
pub(crate) fn asset_browser(mut ui: ResMut<UiBuilder>, mut asset_server: ResMut<AssetServer>, mut dnd: ResMut<AssetDND>, mut local: Local<(Option<PathBuf>, Vec<PathBuf>, usize)>) {
    let (cwd, replay, redo_cursor) = &mut *local;
    let cwd = cwd.get_or_insert_with(|| std::env::current_dir().unwrap().join("game").join("assets"));
    
    let Some(ui) = ui.ui() else { return };

    let res: Option<std::result::Result<(), anyhow::Error>> = ui.window("Asset Browser##asset_browser").build(|| {
        ...
        let mut entries: Vec<(String, PathBuf, FileType, String)> = dir
            ...
            .filter_map(|e| {
                ...
                let extention = e.path().extension().and_then(|v| v.to_owned().into_string().ok()).unwrap_or(String::new());
        ...
        for (i, (name, path, file_type, extention)) in entries.iter().enumerate() {
            ...
            draw.add_rect(
                ...
                if hovered { UiBuilder::S1 } else { UiBuilder::S0 },
            )
            .filled(true)
            .rounding(3.0)
            .build();
